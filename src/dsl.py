...
def extract(source, start_at, end_at):
...
def ext(source, start_at, end_at):
    # this function extracts a text
    if start_at not in source:
        return "NA"
    else:
...
def clext(source, start_at, end_at):
    # this function extracts a text
    if start_at not in source:
        return "NA"
    else:
...
def trim(source, start_at, end_at):
...
def extrair(source, start_at, end_at):
    # this function extracts a text
    if start_at not in source:
        return "na"
    else:
...
def origem_ato(origem_sigla, intdos):

    ato_impugnado_origem = origem_sigla
    if (
        "PRESIDENTE DA REPUBLICA" in intdos
        or "CN;" in intdos
        or "SENADO" in intdos
    ...
    origem_ato_extraido_intdos = origem_ato_extraido_intdos.replace(
        "CORR.GERAL DA JUSTICA DO ", ""
    )
    origem_ato_extraido_intdos = origem_ato_extraido_intdos.replace(
        "PREF. DO MUN. DE ", ""
    )
...
def get_CC(url):
    ...
    html = html.text
    html = extrair(
        html, '<div class="titulo-formulario">', '<section id="mapa-do-site">'
    )
    return html
...
def write_csv_header(nomedoarquivo, string_campos):
    ...
    if nomedoarquivo in os.listdir():
        arquivoaberto = open(nomedoarquivo, mode="r+", encoding="utf-8", newline="")
        html = arquivoaberto.read()
        arquivoaberto.close()

        if lista_de_campos[0] not in html[:100]:
            arquivoaberto = open(nomedoarquivo, mode="w", encoding="utf-8", newline="")
...
def ajustar_nome(string):
    ...
    if " SINDICATO " in string:
        string = (
            string[string.find(" SINDICATO ") + 1 :]
            + " "
            + string[: string.find(" SINDICATO ")]
        )

    if " SOCIEDADE BRASILEIRA " in string:
        string = (
            string[string.find(" SOCIEDADE BRASILEIRA ") + 1 :]
            + " "
            + string[: string.find(" SOCIEDADE BRASILEIRA ")]
        )
    ...
    substituir = [
        ...
        [
            "SINDIFISCO NACIONAL",
            "SIND.NAC. AUDITORES FISCAIS DA RECEITA FEDERAL DO BRASIL SINDIFISCO NACIONAL",
        ],
        ["UNIAO GERAL DOS TRABALHADORES", "UNIAO GERAL DOS TRABALHADORES UGT"],
        [
            "UNIAO NACIONAL DAS INSTITUICOES DE AUTOGESTAO EM SAUDE",
            "UNIAO NACIONAL DAS INSTITUICOES DE AUTOGESTAO EM SAUDE UNIDAS",
        ],
        ["AL/SAO PAULO ADI 2476 EM APENSO", "AL/SAO PAULO"],
        ["ARTIGO 19 BRASIL", "ASSOC. ARTIGO 19 BRASIL"],
        [
            "EDUCAFRO",
            "ASSOC. EDUCACAO E CIDADANIA DE AFRO DESCENDENTES E CARENTES EDUCAFRO",
        ],
    ...
    if string.find("CONFEDERACAO") > 0 and string.find("-") < 20:
        ...
        string = string.replace("  ", " ")
        string = (
            string[string.find("CONFEDERACAO") :]
            + " - "
            + string[: string.find("CONFEDERACAO")]
        )

    if (
        string.find("FEDERACAO") > 0
        and string.find("-") < 20
        and "CONFEDERACAO" not in string
    ):
        string = string.replace("-", "")
        string = string.replace("  ", " ")
        string = string.replace("  ", " ")
        string = (
            string[string.find("FEDERACAO") :]
            + " - "
            + string[: string.find("FEDERACAO")]
        )
    ...
    trocar = [
        ["ASSEMBLEIA LEGISLATIVA DO ESTADO DE ", "AL/"],
...
def solicitar_dados_Juris(classe, numero):
    ...
    inicio = string.find(
        '<a href="#" id="imprimir" onclick="sysImprimir(); return false;">Imprimir</a>'
    )
    return url + ">>>>> \n" + string[inicio:]
...
def gerador_de_lista(path, classe, numeroinicial, numerofinal):
    ...
    for n in range(int(numerofinal) - int(numeroinicial) + 1):
        ...
        nomedoarquivo = (
            path + classe + str(0) * (4 - len(str(numero))) + str(numero) + ".html"
        )
...
def gerar_lista(classe, numeroinicial, numerofinal, path):
    ...
    for item in range(int(numerofinal) - int(numeroinicial) + 1):
        nomedoarquivo = (
            path + classe + ("0") * (4 - len(str(item))) + str(item) + ".html"
        )
        print(nomedoarquivo)
        lista.append(nomedoarquivo)
    return lista
...
def extrai_acordaos_da_string(
    arquivo_a_extrair, path
):  # usar duas contra-barras depois do nome
    ...
    if arquivo_a_extrair in os.listdir(path):
        ...
        if "Nenhum registro encontrado" in acordaos:
            ...
            for decisoes in range(int(n_acordaos)):
                ...
                processo_juris = extrair(
                    acordaos,
                    """<!-- Término do trecho que passa informações para o QueryString (Pesquisa Simultânea de Jurisprudência) --""",
                    "<br />",
                ).upper()
                ...
                if "ADI" in arquivo_a_extrair:
                    processo_juris = processo_juris.replace(
                        "AÇÃO DECLARATÓRIA DE CONSTITUCIONALIDADE", "ADI"
                    )
                ...
                processo_juris = processo_juris.replace(">", "")
                processo_juris = processo_juris.replace(
                    "REFERENDO NA MC", "MC (REFERENDO)"
                )
                processo_juris = processo_juris.replace(
                    "REFERENDO NOS EMB.DECL.", "EMB.DECL. (REFERENDO)"
                )
                processo_juris = processo_juris.replace(
                    "REFERENDO NO AG.REG.", "AG.REG (REFERENDO)"
                )
                ...
                orgao_julgador_acordao = extrair(
                    acordaos, "Órgão Julgador:&nbsp", "<br />"
                )

                publicacao_acordao = extrair(
                    acordaos,
                    """<PRE><span style='font-family:tahoma, verdana, arial, sans-serif;font-size:1.1 em;font-weight:bold'>""",
                    "</PRE>",
                )
                ...
                decisao_juris = extrair(
                    acordaos,
                    """<p><div style="text-align:justify; color: #385260; font-weight: normal; font-size: 11px">""",
                    "</div>",
                )
                ...
                observacao = extrair(
                    acordaos, """<p><strong>Observação</strong></p>""", "</PRE>"
                )
                if (
                    "Acórdão(s) citado(s)" in acordaos
                    and "Nenhum registro encontrado" not in acordaos
                    and "AGUARDANDO INDEXAÇÃO" not in acordaos
                ):
                    ...
                    if observacao != [] and "Número de páginas" in observacao[-1]:
                        observacao[-1] = extrair(
                            observacao[-1], "", "Número de páginas"
                        )
                    lista_processos_citados = []
                    lista_procesoss_citados_com_tema = []
                    for obs in range(len(observacao)):
                        ...
                        for item in range(len(elemento)):
                            ...
                            lista_procesoss_citados_com_tema.append(
                                processo_citado_e_tema
                            )

                doutrina = extrair(
                    acordaos, """<p><strong>Doutrina</strong></p>""", "</PRE>"
                )
...
def extrai_mono_da_string(
    ...
    if arquivo_a_extrair in os.listdir(path):
        ...
        n_monocraticas = monocraticas.count(
            'img src="imagem/bt_imprimirpopup.gif" alt="Imprimir" style="position:relative;left:490px;top:-38px;margin-bottom:-55px;'
        )
        ...
        if "Nenhum registro encontrado" in monocraticas:
            ...
            for decisoes in range(int(n_monocraticas)):
                ...
                relator_juris = extrair(
                    monocraticas, "Relator(a):&nbsp ", "<br />"
                ).upper()
                relator_juris = relator_juris.lstrip(" ")
                relator_juris = relator_juris.lstrip("MIN.")
                relator_juris = relator_juris.lstrip(" ")
                ...
                publicacao_monocratica = extrair(
                    monocraticas,
                    """<pre><span style='font-family:tahoma, verdana, arial, sans-serif;font-size:1.1 em;font-weight:bold'>""",
                    "</pre>",
                )
                ...
                decisao_juris = extrair(
                    monocraticas, """Decisão</strong></p>""", "</pre>"
                )
                ...
                legislacao = extrair(
                    monocraticas, """Legislação</strong></p>""", "</pre>"
                )
                legislacao = legislacao.replace("\t", "")
                legislacao = legislacao.replace("\n", "")

                observacao = extrair(
                    monocraticas, """<p><strong>observação</strong></p>""", "</pre>"
                )
...
def inserir_ocorrencias(lista):
    ...
    for item in lista:
        ...
        print(
            "Inserindo número de ocorrências: "
            + str(contador)
            + " de "
            + str(len(lista))
        )
...
def consolidar_entradas(lista):
    ...
    if len(lista) > 0:
        ...
        for n in range(len(lista) - 1):
            print(
                "Consolidando entradas: " + str(contador) + " de " + str(len(lista) * 2)
            )
        ...
        if item2[0] == item1[0][: len(item2[0])]:
            if (
                ...
                if lista[i + 1][1] > lista[i][1]:
                    iguais.append(
                        (lista[i + 1][0], lista[i + 1][1], lista[i + 1][0], lista[i][1])
                    )
                    lista[i + 1][0] = lista[i][0]
                else:
                    iguais.append(
                        (lista[i][0], lista[i][1], lista[i + 1][0], lista[i + 1][1])
                    )
                    lista[i + 1][0] = lista[i][0]
            elif (
                item2[0] == "CIDADANIA"
                or "OAB" in item2[0]
                or "SIND." in item2[0]
                or "MP DO TRABALHO" in item2[0]
                or "RECEITA FEDERAL" in item2[0]
                or "CONS.NAC." in item2[0]
            ):
                # mantém o que tem mais entradas
                if lista[i + 1][1] > lista[i][1]:
                    iguais.append(
