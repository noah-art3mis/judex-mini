...
- **Point-in-time snapshot — the corpus is not a live mirror.** Each `data/source/processos/<classe>/judex-mini_<classe>_<n>-<n>.json` records the case **as of the moment that `judex executar` (or `judex atualizar`) scraped it**, not as of "now." STF mutates a **processo** continuously after that point: new **andamentos** land (decisões, certidões, intimações, sessões virtuais), new **peças** get attached, the **outcome** can flip from `None` to `concedido` / `denegado` / `prejudicado`, **partes** can change (new ADV substitutes prior counsel, new IMPDO added). None of those updates are visible in our on-disk cache until we explicitly re-scrape the case. This means:
  - Analysis answers are "as of the latest scrape", not "as of today" — a query like "how many HCs are still pending?" is really "how many were pending at last scrape, possibly months ago." Cite the snapshot date in any published artefact.
  - The corpus drifts further from STF's live state with every passing day. Cases scraped a year ago may have completed terminating decisions that aren't reflected in our `outcome` derivation.
  - There is **no automatic refresh** — the operator chooses when to re-scrape. The mechanical paths to refresh: `judex executar -c <classe> -i N -f M --forcar` for a case-id range, or invoke the legacy `judex debug varrer-processos` chain for selective replays. `--forcar` bypasses `handle_fetch_meta`'s skip-if-exists guard.
  - For research-grade work (academic papers, defense filings, comparative panels), pin a specific scrape date by re-scraping the relevant case-id range immediately before publication and freezing/archiving the resulting `data/source/processos/` and `data/derived/pecas-texto/` snapshots. Don't let analyses straddle long re-scrape windows.
