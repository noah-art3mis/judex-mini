...
- **Scope follow-up passes to what you just collected — `--csv` is the scoping knob, `--saida` only locates state.** After a `baixar-pecas` sweep on year-N, the natural next step is `extrair-pecas` to flip those bytes into text. Always pair them: `extrair-pecas --csv <sweep>/cases.csv --saida <sweep>/`. The 2026-04-30 design hardening makes `judex/sweeps/peca_cli.py:resolve_targets` *raise* `ValueError` on bare invocation (no `--retentar-de`, no `--csv`, no range, no filter), so both `baixar-pecas` and `extrair-pecas` exit-2 with a clean error instead of falling through to corpus-wide enumeration — the historical footgun. Why the guard exists: two compounding cliffs hit if `--csv` isn't passed. (1) Bare resolve walks the full corpus target list (`collect_peca_targets` over all `data/source/processos/HC/`), so a 15k-record year-sweep follow-up walks 120k records — 8× wasted work just from scope. (2) The atomic state-write in `judex/sweeps/peca_store.py` rewrites the entire `pdfs.state.json` per record; a 53 MB corpus state caps `extrair-pecas` at ~0.13 rec/s on WSL2 while a `--csv`-scoped pass on a fresh 8 MB state hits ~3.5 rec/s — a 27× rate gap on top of the 8× scope gap. Together: scoped finishes in ~70 min, corpus-wide ETAs to **9 days**. The guard is pinned by `tests/unit/test_peca_cli.py::test_resolve_raises_when_no_scope_specified`. Same scoping rule applies to analysis: when "I want to look at what we just collected", point DuckDB/pandas at `<sweep>/pdfs.log.jsonl` or rebuild a year-scoped warehouse — don't grep the corpus-wide warehouse for a single year.
