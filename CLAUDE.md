...
- **Recommend `tail -f <run_dir>/launcher-stdout.log` as the canonical live sweep monitor** before reaching for anything fancier. All three pipeline stages (`varrer-processos`, `baixar-pecas`, `extrair-pecas`) write the same per-record `[N/total] ...` + periodic `[progress] ok=… fail=… · X.XX proc/s · eta Y min` shape to the nohup'd launcher log — `tail -f` shows liveness, throughput, ETA, and recent errors in one view. Pair with `pgrep -af <run_label>` for liveness if needed. Sharded runs have a richer alternative (`judex probe --out-root <dir> --watch N`); regime-trajectory analysis (cliffs / SSL-EOF storms) is post-hoc via `judex analisar-regimes <run_dir>`. Don't write bespoke monitor scripts when `tail -f` already carries the live signal.
