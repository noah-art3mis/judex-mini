# `deprecated/` — frozen Selenium scraper

This subtree holds the original Selenium-based STF scraper, retired
2026-04-17. The HTTP backend (`src/scraping/scraper.py`) is now the only
...
```
_deprecated/
├── scraper.py                      # was src/scraping/scraper.py — orchestrator
├── utils/
...
`src/scraping/extraction/` — they have no Selenium dependency.
...
```python
from deprecated.scraper import run_scraper
```

## Hard-removal criterion

If a year passes with no one importing from `deprecated/`, the
