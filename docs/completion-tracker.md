...
outlier recovery + HC 2023 bytes second-pass + warehouse rebuild).
...
| 2023 | 12,644 | 11,129 | ✅ 21,329 / 21,430 = 99.5% | ✅ 21,329 / 21,430 = 99.5% | ✅ 21,091 / 21,186 = 99.6% |    21,186 |
...
retry needed). HC 2023 bytes 67% → 99.5% (single 55-minute direct-IP
second-pass against 10,371 cases; `--prever` over-estimated by 6×).
HC 2021 bytes/text 0.5%/1% → 96.1%/96.3% (chain v2 + delta re-run).
HC 2020 partial bump from outlier recovery (3 cached + 1 newly-
refetched outlier OCR'd via local Tesseract; 4 lost — 3 orphan URLs
from the May 5-11 corpus deletion + 1 persistent STF-empty).
HC 2026 bytes 73% → 99.8% (incremental sweeps + recuperar drains).
HC 2025 / 2022 essentially unchanged. **2013 is a new row** — sparse
legacy fragment of cases whose `data_protocolo_iso` fell in 2013
(likely re-distributed or re-classified cases); not a target.
...
The **2026 → 2021 ladder is now fully closed for peças** (all six
years ≥96% on both bytes and text after the 2026-05-12 chain-delta
re-run + HC 2024 delta-driven anomaly closure + HC 2023 bytes
second-pass). Remaining priorities are the older case-id-space gaps.

1. **2019, 2018, 2017 cases** — ~37k missing cases combined; three
   sequential year sweeps; closes the 2017–2022 hole. Followed by
   the matching peça sweeps (~13–15k URLs each, same shape as
   2024/2023/2022).
2. **2020 cases** — 4,493 / 15,754 widths on disk; ~11,300 missing.
