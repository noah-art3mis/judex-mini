...
|-------------|------------------------------------------------------------------------|-------------------------------------------------------------------------------|------------------------------------------------------------------------------------------|------------------------------------|
| **2022-12-19** | **DJe content URL migration** — substantive DJe entries (Decisão / Acórdão / Despacho) moved their *content URLs* from inline `verDecisao.asp?...` links to a redirect message pointing at `digital.stf.jus.br/publico/publicacoes`. The legacy listing endpoint at `portal.stf.jus.br/servicos/dje/listarDiarioJustica.asp?classe=X&numero=N` **still serves data** for every year (HTTP 200, ~55 KB), including post-migration metadata (DJ date, DJ number) — only the *content URLs* changed. Procedural entries (Distribuição) still use the legacy `abreDetalheDiarioProcesso(...)` JS-clickable form. | 2026-04-21 warehouse build-stats validation flagged `publicacoes_dje: 0.0% (threshold ≥ 5.0%)`. 2026-05-01 reconnaissance identified the actual cause: our `parse_dje_listing` parser hard-requires `abreDetalheDiarioProcesso(...)` `onclick` JS calls and silently discards the redirect-form entries that STF now uses for substantive publications. | **Diagnosed; fix proposed in [ADR-0003](adr/0003-surface-3-dje-capture-path.md).** Phase 1 (1-2 h): patch `extraction/dje.py` to also capture redirect entries — recovers full DJe metadata (date, DJ number, redirect URL) without infrastructure changes. Phase 2 (deferred): Playwright-driven WAF traversal *only if* DJe-only content text proves analytically necessary beyond what surfaces 1+2 already cover. AWS WAF on `digital.stf.jus.br/publico/publicacoes` confirmed (HTTP 202 + `token.awswaf.com` JS challenge), but no public API analogue to surface 2 exists, so the WAF is the only path to content if Phase 2 fires. | ⚠ Phase 1 proposed; 0% DJe metadata until it lands |
